...
Theorem extreal_sum_image_val_map_assignments_combine_dependent_inner_set:
  ∀fg ns1 ns2 excl_val_map1 excl_val_map2 f x_choice.
    ...
    (∀x. x ∈ val_map_assignments fg ns1 excl_val_map1 ⇒
         FDOM (excl_val_map2 x) ⊆ ns2) ∧
    DISJOINT ns1 ns2 ∧
    ((∀x y.
        x ∈ val_map_assignments fg ns1 excl_val_map1 ∧
        y ∈ val_map_assignments fg ns2 (excl_val_map2 x) ⇒
        f x y ≠ +∞) ∨
     ∀x y.
       x ∈ val_map_assignments fg ns1 excl_val_map1 ∧
       y ∈ val_map_assignments fg ns2 (excl_val_map2 x) ⇒
    ...
    (∀x n. x ∈ val_map_assignments fg ns1 excl_val_map1 ∧
           n ∈ FDOM (excl_val_map2 x) ⇒
           LENGTH (excl_val_map2 x ' n) = get_variable_length_map fg ' n) ⇒
    ∑ (λx.
         ∑ (λy. f x y)
...
Proof
  ...
  >- (simp[Abbr ‘LHS’]
      ...
      >> simp[]
      >> rpt strip_tac
      >> qabbrev_tac ‘g = λy. f x y’
      >> simp[]
      >> DEP_PURE_ONCE_REWRITE_TAC[GSYM extreal_sum_image_change_excl_val_map]
      >> conj_tac
      >- (simp[Abbr ‘g’]
          >> gvs[])
      >> simp[]
...
Proof
  ...
  >- (gvs[]
      ...
      (* We can simplify DRESTRICT val_map (ns1 DELETE dst) to instead restrict
         to ns1 because we are already restricted to ns2 and ns2 already doesn't
         have dst in it.
.
      ...
      >> Q.SUBGOAL_THEN
          ‘∀val_map. val_map_assignments fg ns2
                     ...
                     = val_map_assignments fg ns2 (DRESTRICT val_map ns1)’
          (fn th => simp[th])
      >- (rpt strip_tac
          ...
          >> rpt strip_tac
          >> Cases_on ‘x ∈ ns1’ >> simp[]
          >> gvs[Abbr ‘nsf’, Abbr ‘ns1’]
          >> gvs[subtree_def]
          >> ‘get_path (get_underlying_graph fg) src dst = [src; dst]’ by metis_tac[adjacent_get_path, adjacent_SYM]
          >> gvs[]
         )
      >> Q.SUBGOAL_THEN
          ‘∀val_map.
             val_map ∈ val_map_assignments fg ns1 excl_val_map ⇒
             DRESTRICT val_map ns1 = val_map ’
