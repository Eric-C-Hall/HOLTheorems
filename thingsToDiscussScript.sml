...
(*                                                                            *)
(* Does not use the Viterbi algorithm for decoding, but rather uses a         *)
(* combinatorial method implemented by the below algorithm                    *)
(* -------------------------------------------------------------------------- *)
...
(*                                                                            *)
(* The problem under consideration requries fast algorithms, so typical       *)
...
(*                                                                            *)
(* To minimize complexity and improve speed, focuses on single-bit            *)
(* correction.                                                                *)
(*                                                                            *)
(* Very specific choice of generating polynomials: (1, 1, 1) and (1, 0, 1)    *)
(*                                                                            *)
(* Unfolded over entire word in order to improve speed, transforming from     *)
(* sequential into combinatorial                                              *)
(*                                                                            *)
(* Uses custom, seemingly novel, combinatorial method for decoding.           *)
(*                                                                            *)
(* Four received bits can uniquely identify the two bits which must have      *)
(* generated them. Thus, easy to decode uniquely in the case where we know    *)
(* that no errors have occurred.                                              *)
(*                                                                            *)
(* Firstly, decode uniquely as if there were no errors, and overlap the       *)
(* decoding modules to create another decoding option as if there were no     *)
(* errors.                                                                    *)
(*                                                                            *)
(* From this pair of decoding options, it is possible to reconstruct the      *)
(* decoded bit under the assumption of only a single error.                   *)
(*                                                                            *)
(* Can correct one error per 5 bits, if well spaced.                          *)
(*                                                                            *)
(* In first stage, decodes uniquely as if there were no errors                *)
(* In second stage,                                                           *)
(*                                                                            *)
(* -------------------------------------------------------------------------- *)
(* Questions/things to discuss:                                               *)
